...
struct HealthData: Codable, Identifiable {
    ...
    // MARK: - Computed Properties for Trend Analysis
...
//        allData.sort { $0.date > $1.date }
    ...
    func getEntriesFromLastDays(days: Int) -> [HealthData] {
        let now = Date()
        guard let cutoff = Calendar.current.date(byAdding: .day, value: -days, to: now) else { return [] }
        return getAllData().filter { $0.date >= cutoff }
    }
    
    func getEntriesBetweenDays(startDays : Int, endDays: Int) -> [HealthData] {
        let now = Date()
        let current = Calendar.current
        guard let startDate = current.date(byAdding: .day, value: -startDays, to: now) else {return []}
        guard let endDate = current.date(byAdding: .day, value: -endDays, to: now) else {return []}
        return getAllData().filter {$0.date >= endDate && $0.date <= startDate}
    }
